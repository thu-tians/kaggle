...
def prepare_data(marketdf, newsdf):
    ...
    marketdf = marketdf.loc[marketdf['returnsClosePrevRaw1'] - marketdf['returnsOpenPrevRaw1'] < 2]
    ...
    data = pd.merge(marketdf, newsgp, how='left', on=['time', 'assetCode'], copy=False) #, right_on=['time', 'assetCodes'])market_train_df['close_open_ratio'] = np.abs(market_train_df['close']/market_train_df['open'])
    data = data.loc[data['bartrend'] < 10]
    # join news reports to market data, note many assets will have many days without news data
    return data #, right_on=['time', 'assetCodes'])
...
params = {"objective" : "binary",
          ...
          "num_leaves" : 125,#125, # originally 60
          "max_depth": -1,
          "learning_rate" : 0.0005,   # originally .01
          "bagging_fraction" : 0.9,  # subsample
          "feature_fraction" : 0.9,  # colsample_bytree
          "bagging_freq" : 5,        # subsample_freq
          "bagging_seed" : 2018,#2018,
          "verbosity" : -1 }
...
def post_scaling(df):
    # max1 ,min1 = np.max(df),np.min(df)
    # df = ((df - min1)/(max1-min1))*0.04
    mean, std = np.mean(df), np.std(df)
    df = (df - mean)/ (std * 8)
    return np.clip(df,-0.1,0.1)
    # return df
def set_zero(df):
    df = df * 0
    return df
...
for marketdf, newsdf, predtemplatedf in preddays:
    ...
    if cdf['time'].max()< 20190000:
        predsdf = pd.DataFrame({'ast':cdf['assetCode'],'conf':post_scaling(preds)})
    else:
        predsdf = pd.DataFrame({'ast':cdf['assetCode'],'conf':set_zero(preds)})
    # predsdf['conf'].values = predsdf['conf'].values * 0.1
